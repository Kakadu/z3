...
void for_each_ast(ForEachProc & proc, ast_mark & visited, ast * n, bool visit_parameters = false) {
    ...
    while (!stack.empty()) {
        ...
        case AST_QUANTIFIER:
            ...
            break;
        case AST_LAMBDA:
            if (!for_each_ast_args(stack, visited, to_lambda(curr)->get_num_patterns(),
                                   to_lambda(curr)->get_patterns())) {
                break;
            }
            if (!for_each_ast_args(stack, visited, to_lambda(curr)->get_num_no_patterns(),
                                    to_lambda(curr)->get_no_patterns())) {
                break;
            }
            if (!visited.is_marked(to_lambda(curr)->get_expr())) {
                stack.push_back(to_lambda(curr)->get_expr());
                break;
            }
            proc(to_lambda(curr));
            visited.mark(curr, true);
...
struct for_each_ast_proc : public EscapeProc {
    ...
    void operator()(lambda * n) { operator()(static_cast<lambda *>(n)); }
};
...
public:
    static T* recurse(Visitor & visit, ast * aArg) {
        ...
        while (!stack.empty()) {
            ...
            case AST_LAMBDA: {
                lambda * lambda_ast = to_lambda(a);
                ptr_vector<T> decl_types;

                if (true /*recurse_quantifier*/) {
                    args = (ast * const *) lambda_ast->get_decl_sorts();
                    arity = lambda_ast->get_num_decls();
                    ast* body = lambda_ast->get_expr();
                    ...
                    if (!memoize.find(body, result)) {
                        ...
                        result = visit.mk_quantifier(lambda_ast, decl_types, result);
                        memoize.insert(a, result);
                        stack.pop_back();
                    }
                }
